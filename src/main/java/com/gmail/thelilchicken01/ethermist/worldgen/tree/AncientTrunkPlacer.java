        ...
        for (int x2 = -1; x2 <= 2; x2++) {
            for (int y2 = -1; y2 <= 2; y2++) {
                if ((x2 < 0 || x2 > 1 || y2 < 0 || y2 > 1) && random.nextInt(3) <= 1) {
                    int branchSize = random.nextInt(4) + 3;
                    int branchTop = treeMaxPos + 2 - random.nextInt(3);

                    for (int branchCount = 0; branchCount < branchSize; branchCount++) {
                        BlockPos currentPos = new BlockPos(xPos + x2, branchTop - branchCount, zPos + y2);
                        this.placeLog(level, blockSetter, random, currentPos, config);
                    }

                    list.add(new FoliagePlacer.FoliageAttachment(new BlockPos(xPos + x2, branchTop + 1, zPos + y2), 0, false));
                }
            }
        }
